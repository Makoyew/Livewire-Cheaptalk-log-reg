        ...
        <div class="row">
            <div class="col">
                <div class="col-md-6 mx-auto">
                    <button class="raise3 btn bg-warning float-end mb-2" data-bs-toggle="modal" data-bs-target="#newCategoryModal">New Category</button>
                    <div class="mt-3">
                        <table class="table table-bordered table-hover borderclr shadow">
                            ...
                            <tbody style="background-color: rgba(255, 255, 0, 0.507);">
                                @foreach ( $categories as $category )
                                    <tr>
                                        <td class="text-center">{{ $category->category }}</td>
                                        <td>{{ $category->remarks }}</td>
                                        <td><button class="btn bg-danger text-white" data-bs-toggle="modal" data-bs-target="#deleteCategoryModal" wire:click="rmvCat({{ $category->id }})">Delete</button></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
   ...
   <div wire:ignore.self class="modal fade" id="newCategoryModal" tabindex="-1" data-bs-backdrop="static" aria-labelledby="newCategory" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning bghdcolor theadclr">
                    <h1 class="modal-title fs-5" id="newCategory">Add New Category</h1>
                </div>
                <div class="modal-body bgcolor" style="background-color: rgba(255, 255, 0, 0.507);">
                ...
                <div class="modal-footer" style="background-color: rgba(255, 255, 0, 0.507);">
                    <button type="button" class=" raise3 bg-warning btn" data-bs-dismiss="modal">Close</button>
                    <button type="button" class="raise3 bg-warning btn addcatebtnclr" data-bs-dismiss="modal" wire:click="store()">Add Category</button>
                </div>
            </div>
        </div>
    </div>
